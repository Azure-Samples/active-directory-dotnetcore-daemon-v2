...
Function ConfigureApplications
{
   ...
   $clientAadApplication = New-MgApplication -DisplayName "daemon-console" `
                                                      -Web `
                                                      @{ `
    ...
    #Update-MgApplication -ApplicationId $currentAppObjectId -IdentifierUris @("https://$tenantName/daemon-console")
    ...
    Write-Host "Successfully registered and configured that app registration for 'daemon-console' at `n $clientPortalUrl" -ForegroundColor Green 
    
    # Update config file for 'client'
    # $configFile = $pwd.Path + "\..\daemon-console\appsettings.json"
    $configFile = $(Resolve-Path ($pwd.Path + "\..\daemon-console\appsettings.json"))
    
    $dictionary = @{ "TenantId" = $tenantName;"ClientId" = $clientAadApplication.AppId;"ClientSecret`":" = $clientAppKey };
