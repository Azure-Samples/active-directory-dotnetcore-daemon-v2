...
Function Cleanup
{
    ...
    {
        Write-Host "Unable to remove the application 'TodoList-webapi-daemon-v2' . Try deleting manually." -ForegroundColor White -BackgroundColor Red
    }

    Write-Host "Making sure there are no more (TodoList-webapi-daemon-v2) applications found, will remove if needed..."
    $apps = Get-MgApplication -Filter "DisplayName eq 'TodoList-webapi-daemon-v2'"
    ...
    try
    {
        Get-MgServicePrincipal -filter "DisplayName eq 'TodoList-webapi-daemon-v2'" | ForEach-Object {Remove-MgServicePrincipal -ApplicationId $_.Id -Confirm:$false}
    }
    catch
    {
        Write-Host "Unable to remove ServicePrincipal 'TodoList-webapi-daemon-v2' . Try deleting manually from Enterprise applications." -ForegroundColor White -BackgroundColor Red
    }
    ...
    {
        Write-Host "Unable to remove the application 'daemon-console-v2' . Try deleting manually." -ForegroundColor White -BackgroundColor Red
    }

    Write-Host "Making sure there are no more (daemon-console-v2) applications found, will remove if needed..."
    $apps = Get-MgApplication -Filter "DisplayName eq 'daemon-console-v2'"
    ...
    try
    {
        Get-MgServicePrincipal -filter "DisplayName eq 'daemon-console-v2'" | ForEach-Object {Remove-MgServicePrincipal -ApplicationId $_.Id -Confirm:$false}
    }
    catch
    {
        Write-Host "Unable to remove ServicePrincipal 'daemon-console-v2' . Try deleting manually from Enterprise applications." -ForegroundColor White -BackgroundColor Red
    }
